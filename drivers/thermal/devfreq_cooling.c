...
static u32 devfreq_cooling_get_actual_power(struct thermal_cooling_device *cdev)
{
	struct devfreq_cooling_device *dfc = cdev->devdata;
...
static u32 devfreq_cooling_state2power(struct thermal_cooling_device *cdev,
		unsigned long state)
{
	struct devfreq_cooling_device *dfc = cdev->devdata;

	return dfc->power_table[state];
}

static unsigned long
devfreq_cooling_power2state(struct thermal_cooling_device *cdev, u32 power)
{
	struct devfreq_cooling_device *dfc = cdev->devdata;
	struct devfreq *df = dfc->devfreq;
	int i;
...
static struct thermal_cooling_device_ops const devfreq_cooling_ops = {
	...
	.get_actual_power = devfreq_cooling_get_actual_power,
